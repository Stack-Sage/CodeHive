...
export default function EducatorDashboard() {
  ...
  const isEducator = userRole === "teacher" || user?.roles?.includes("educator");
  const isStudent = userRole === "student" || user?.roles?.includes("student");
  ...
  return (
    <section className="w-full max-w-screen-2xl mx-auto px-2 md:px-6 py-8">
      ...
      <div className="flex flex-col lg:flex-row gap-8">
        ...
        >
          <motion.div transition={{ duration: 0.5, delay: 0.1 }} whileHover={{ scale: 1.03 }}>
            <ProfileCard user={user} />
          </motion.div>
          <motion.div transition={{ duration: 0.5, delay: 0.15 }} whileHover={{ scale: 1.03 }}>
            <AccountInfo />
          </motion.div>
          <motion.div transition={{ duration: 0.5, delay: 0.2 }} whileHover={{ scale: 1.03 }}>
            <ProfileCompletionBar percent={user?.profileCompletion} />
          </motion.div>
          <motion.div transition={{ duration: 0.5, delay: 0.25 }} whileHover={{ scale: 1.03 }}>
            <SiteStatsSidebar />
          </motion.div>
          <motion.div transition={{ duration: 0.5, delay: 0.3 }} whileHover={{ scale: 1.03 }}>
            <QuickActions />
          </motion.div>
        </motion.div>
        ...
        <div className="flex-1 flex flex-col gap-8 mx-auto w-full lg:w-auto">
          ...
          <motion.div transition={{ duration: 0.5, delay: 0.1 }} whileHover={{ scale: 1.02 }}>
            <StatsGrid stats={dashboardStats} />
          </motion.div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <motion.div transition={{ duration: 0.5, delay: 0.15 }} whileHover={{ scale: 1.02 }}>
              <ProfileClicksLine stats={dashboardStats} />
            </motion.div>
            {isEducator && (
              <motion.div transition={{ duration: 0.5, delay: 0.18 }} whileHover={{ scale: 1.02 }}>
                <SessionsDoughnut stats={dashboardStats} />
              </motion.div>
            )}
          </div>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
            <motion.div transition={{ duration: 0.5, delay: 0.2 }} whileHover={{ scale: 1.02 }}>
              <SearchResultStat count={dashboardStats.searchAppearances} />
            </motion.div>
            <motion.div transition={{ duration: 0.5, delay: 0.22 }} whileHover={{ scale: 1.02 }}>
              <MessagesStat sent={dashboardStats.messagesSent} received={dashboardStats.messagesReceived} />
            </motion.div>
          </div>
          {isEducator && (
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
              <motion.div transition={{ duration: 0.5, delay: 0.25 }} whileHover={{ scale: 1.02 }}>
                <SessionHoursStat hours={dashboardStats.sessionHours} />
              </motion.div>
              <motion.div transition={{ duration: 0.5, delay: 0.28 }} whileHover={{ scale: 1.02 }}>
                <TopSkillsChart skills={dashboardStats.skills} />
              </motion.div>
            </div>
          )}
          <motion.div transition={{ duration: 0.5, delay: 0.3 }} whileHover={{ scale: 1.02 }}>
            <ExtraStats stats={dashboardStats} />
          </motion.div>
          {isEducator && (
            <>
              <motion.div transition={{ duration: 0.5, delay: 0.32 }} whileHover={{ scale: 1.02 }}>
                <StudentFeedbackSummary feedback={dashboardStats.feedbackSummary} />
              </motion.div>
              <motion.div transition={{ duration: 0.5, delay: 0.34 }} whileHover={{ scale: 1.02 }}>
                <ReviewsList reviews={dashboardStats.reviews} />
              </motion.div>
              <motion.div transition={{ duration: 0.5, delay: 0.36 }} whileHover={{ scale: 1.02 }}>
